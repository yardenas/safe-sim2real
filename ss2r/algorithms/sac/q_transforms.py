...
class PessimisticCostUpdate(QTransformation):
    def __call__(
        ...
        # We do not use transitions.discount as the backup policy is trained with this
        # Q transformation, with data that is generated by model_env. In model_env, we
        # terminate when there are interventions, but the backup policy is agnostic to
        # these intervension and therefore also to their corresponding terminations.
