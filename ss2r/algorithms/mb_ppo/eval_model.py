...
def run_policy(env, policy_fn, steps=1000, key=jax.random.PRNGKey(0)):
    ...
    return jnp.stack(rewards), jnp.stack(actions)


def evaluate_model(model_apply_fn, obs, actions, key, horizon=15):
    pred_rewards = []
    current_state = obs[-horizon - 1]
...
def main(run_id):
    ...
    ppo_network = mb_ppo_networks.make_mb_ppo_networks(
        obs_size, action_size, normalize_fn, model_hidden_layer_sizes=(512, 512)
    )
    ...
    model = ModelBasedEnv(
        ...
        normalizer_params=normalizer_params,  # Assuming normalizer params are part of the model
        action_size=env.action_size,
        safety_budget=float("inf"),  # Set to your desired safety budget
    ...
    horizon = 15
    obs, actions = run_policy(env, policy, steps=horizon, key=key)
    # === Evaluate model over final `horizon` steps ===
    prediction = evaluate_model(model.step, obs, actions, key, horizon=horizon)
    plot(obs, prediction, "rewards.png")


if __name__ == "__main__":
    run_id = "bz92qyby"  # <-- Replace with your actual run ID
    main(run_id)
