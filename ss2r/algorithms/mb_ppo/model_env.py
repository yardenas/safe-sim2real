...
class ModelBasedEnv(envs.Env):
    ...
    def step(self, state: base.State, action: jax.Array) -> base.State:
        ...
        # Log initial shapes
        print(
            f"INPUT - state.reward shape: {state.reward.shape if hasattr(state.reward, 'shape') else 'scalar'}"
        )
        print(
            f"INPUT - state.info['cost'] shape: {state.info['cost'].shape if 'cost' in state.info and hasattr(state.info['cost'], 'shape') else 'not available'}"
        )
        print(
            f"INPUT - state.obs shape: {state.obs.shape if hasattr(state.obs, 'shape') else 'scalar'}"
        )
        print(
            f"INPUT - action shape: {action.shape if hasattr(action, 'shape') else 'scalar'}"
        )
        ...
        input_reward_shape = (
            state.reward.shape if hasattr(state.reward, "shape") else None
        )
        ...
        (
            ...
            self.normalizer_params, self.model_params, obs, action
        )
        ...
        if self.ensemble_selection == "random":
            ...
            print(
                f"AFTER RANDOM - next_obs: {next_obs.shape}, reward: {reward.shape}, cost: {cost.shape}"
            )
            ...
            print(
                f"AFTER MEAN - next_obs: {next_obs.shape}, reward: {reward.shape}, cost: {cost.shape}"
            )
            ...
            # Log shapes
            print(
                f"AFTER PESSIMISTIC - next_obs: {next_obs.shape}, reward: {reward.shape}, cost: {cost.shape}"
            )
        else:
        ...
        # Before squeezing
        print(
            f"BEFORE SQUEEZE - reward shape: {reward.shape}, cost shape: {cost.shape}"
        )
        ...
        if input_reward_shape is not None and input_reward_shape != reward.shape:
            if reward.size == 1:  # If we have a scalar or single element
                print(
                    f"BROADCASTING reward from {reward.shape} to {input_reward_shape}"
                )
                reward = jnp.broadcast_to(reward.reshape((1,)), input_reward_shape)
            # Handle the (512, 1) to (512,) case
            elif reward.shape[0] == input_reward_shape[0] and reward.ndim > len(
                input_reward_shape
            ):
                ...
                print(
                    f"WARNING: Cannot reshape reward from {reward.shape} to {input_reward_shape}"
                )

        if (
            input_reward_shape is not None
            and "cost" in state.info
            and input_reward_shape != cost.shape
        ):
            ...
            # Handle the (512, 1) to (512,) case
            elif cost.shape[0] == input_reward_shape[0] and cost.ndim > len(
                input_reward_shape
            ):
                ...
                print(
                    f"WARNING: Cannot reshape cost from {cost.shape} to {input_reward_shape}"
                )
        ...
        print(
            f"accumulated_cost_for_transition shape: {accumulated_cost_for_transition.shape}"
        )
        ...
        print(
            f"FINAL - next_obs: {next_obs.shape}, reward: {reward.shape}, done: {done.shape}"
        )
